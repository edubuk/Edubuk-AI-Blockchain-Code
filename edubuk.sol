...
//Login Process
function logincompany(string memory email) public view returns (string memory) {
    ...
    return (is_company[msg.sender]) ? "company" : "employee"; // returns account type
}

function logininstitute(string memory email) public view returns (string memory) {
    // checking the function caller's wallet address from global map containing email address mapped to wallet address
    require(
    msg.sender == email_to_address[email],
    "error: incorrect wallet address used for signing in"
    );
    return (is_institute[msg.sender]) ? "institute" : "student"; // returns account type
}

function loginexpert(string memory email) public view returns (string memory) {
    // checking the function caller's wallet address from global map containing email address mapped to wallet address
    require(
    msg.sender == email_to_address[email],
    "error: incorrect wallet address used for signing in"
    );
    return (is_expert[msg.sender]) ? "expert" : "learner";
}
